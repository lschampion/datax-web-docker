...
RUN tar -xf "${USR_PROGRAM_DIR}/source_dir/${DATAX_PACKAGE}" -C "${USR_PROGRAM_DIR}"  \
 && rm -rf  "${DATAX_HOME}/bin/*" \
...
RUN tar -xf "${USR_PROGRAM_DIR}/source_dir/${DATAX_WEB_POACKEAGE}" -C "${USR_PROGRAM_DIR}"  \
 && mv "${USR_PROGRAM_DIR}/datax-web-2.1.2"   "${USR_PROGRAM_DIR}/datax-web" \
# && ls -l "${USR_PROGRAM_DIR}/datax-web/bin" \
 && "${DATAX_WEB_HOME}/bin/install.sh" --force \
 && cp -f "${USR_PROGRAM_DIR}/source_dir/application.yml" "${DATAX_WEB_HOME}/modules/datax-admin/conf/" \
